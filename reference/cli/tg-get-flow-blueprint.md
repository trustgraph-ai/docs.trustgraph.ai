...
```
**Solution**: Check available classes with `tg-show-flow-blueprintes` and verify the class name.
...
mkdir -p flow-blueprint-backups/$(date +%Y%m%d)
tg-show-flow-blueprintes | awk '{print $1}' | while read class; do
    if [ "$class" != "flow" ]; then  # Skip header
        tg-get-flow-blueprint -n "$class" > "flow-blueprint-backups/$(date +%Y%m%d)/$class.json"
    fi
done
```
...
mkdir -p flow-blueprintes
tg-get-flow-blueprint -n "main-flow" > flow-blueprintes/main-flow.json
git add flow-blueprintes/main-flow.json
...
This command uses the [Flow API](../apis/api-flow) with the `get-blueprint` operation to retrieve flow blueprint definitions.
