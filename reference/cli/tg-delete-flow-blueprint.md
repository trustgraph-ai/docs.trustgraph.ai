...
tg-show-flow-blueprintes | grep "target-flow"
...
# 5. Verify deletion
tg-show-flow-blueprintes | grep "target-flow" || echo "Flow blueprint deleted successfully"
```
...
tg-show-flow-blueprintes | grep "target-blueprint"
```
...
tg-show-flows | grep "target-blueprint"
```
...
```
**Solution**: Verify the flow blueprint exists with `tg-show-flow-blueprintes`.
...
backup_dir="flow-blueprint-backups/$(date +%Y%m%d-%H%M%S)"
mkdir -p "$backup_dir"
...
if ! tg-show-flow-blueprintes | grep -q "$flow_class"; then
...
for class in "${classes_to_delete[@]}"; do
    ...
    # Check if exists
    if ! tg-show-flow-blueprintes | grep -q "$class"; then
...
This command uses the [Flow API](../apis/api-flow) with the `delete-blueprint` operation to remove flow blueprint definitions.
...
tg-show-flow-blueprintes | grep "deleted-class"

# Verify API connectivity
tg-show-flow-blueprintes > /dev/null && echo "API accessible"
```
